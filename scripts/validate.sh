...
if command -v pytest &> /dev/null; then
    if pytest --cov=src --cov-fail-under=70 -q &> /tmp/coverage_output.txt; then
        ...
        print_error "Run: pytest --cov=src --cov-report=html"
        EXIT_CODE=$((EXIT_CODE + 8))
    fi
else
